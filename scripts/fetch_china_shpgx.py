...
Runs weekly on GitHub Actions (Mondays 06:00 UTC). Scrapes 5 series from SHPGX,
...
Differences from the Mac version (shpgx_fetch.py v6.2):
  ...
  * Per-series row trimming: keep only last 365 days to cap file size
...
Usage on GitHub Actions:
    Triggered by .github/workflows/china-weekly.yml (cron Mondays 06:00 UTC)
"""
...
KEEP_DAYS = 365  # per-series row trim window
...
def dedupe_and_trim(rows, keep_days=KEEP_DAYS):
    """Sort desc, dedupe by (date, region, grade), trim to last N days."""
    if not rows:
        return []
    ...
    for r in rows:
        ...
        seen.add(k)
        deduped.append(r)
    # Trim: keep rows whose date >= (today - keep_days)
    cutoff = (TODAY - timedelta(days=keep_days)).isoformat()
    trimmed = [r for r in deduped if r.get('date', '') >= cutoff]
    return trimmed
...
async def main():
    ...
    result = {
        ...
        "notes": "Generated by fetch_china_shpgx.py on GitHub Actions. Weekly Mondays 06:00 UTC.",
        "data": {},
    }
    ...
    async with async_playwright() as p:
        ...
        for s in SERIES:
            ...
            if trimmed:
                print(f"  -> {s['key']}: {len(trimmed)} rows after dedupe+trim "
                      f"({trimmed[-1]['date']} -> {trimmed[0]['date']})")
            else:
                print(f"  -> {s['key']}: 0 rows after dedupe+trim")
