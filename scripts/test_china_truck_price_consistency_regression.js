...
assert(js.includes("String(r.region||r.raw?.region||'').includes('华北')"), 'China truck helper must prefer SHPGX North China / 华北 aggregate');
...
assert(js.includes('const truckData = chinaNorthTruckSeries();'), 'Gas Price Chain must use same North China truck helper');
